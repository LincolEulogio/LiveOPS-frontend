...
export const CommandItem = ({ command, onAck }: Props) => {
    ...
    return (
        <div className={cn(
            ...
            isFromMe ? "bg-indigo-500/5 border-indigo-500/20" : "bg-card-bg border-card-border"
        )}>
            {/* Header */}
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 overflow-hidden">
                    <div className={cn(
                        "p-1.5 rounded-lg",
                        isFromMe ? "bg-indigo-500/10 text-indigo-400" : "bg-card-border text-muted"
                    )}>
                        <UserIcon size={14} />
                    </div>
                    <span className={cn(
                    ...
                    {command.targetRole && (
                        <div className="flex items-center gap-1 px-1.5 py-0.5 bg-background border border-card-border rounded text-[10px] font-bold text-muted uppercase tracking-tight">
                            <ShieldCheck size={10} />
            ...
            {command.responses && command.responses.length > 0 && (
                <div className="mt-1 space-y-1.5 border-t border-card-border/50 pt-2">
                    {command.responses.map((resp) => (
                        <div key={resp.id} className="flex items-center gap-2 text-[11px]">
                            <Reply size={12} className="text-muted rotate-180" />
                            <span className="text-foreground font-bold">{resp.responder?.name}:</span>
                            <span className="text-emerald-400 bg-emerald-500/5 px-1.5 rounded border border-emerald-500/10 leading-none py-0.5">
                                {resp.response}
                            </span>
                            {resp.note && <span className="text-muted italic">"{resp.note}"</span>}
                        </div>
                    ))}
                </div>
            )}
