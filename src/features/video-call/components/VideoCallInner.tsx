...
export function VideoCallInner({
  roomId,
  userId,
  userName,
  isHost,
  ...
  const [activeReactions, setActiveReactions] = useState<
    { emoji: string; id: number; senderName?: string }[]
  >([]);
      ...
      try {
        ...
        if (data.type === 'reaction') {
          setActiveReactions((prev) => [
            ...prev,
            { emoji: data.emoji, id: data.id, senderName: data.senderName },
          ]);
          setTimeout(
          ...
          setChatMessages((prev) => [
            ...prev,
            {
              ...
              from: { name: data.senderName, isLocal: false },
            },
          ]);
  ...
  const stripTracks = useMemo(
    () =>
      featuredTrack
        ? cameraTracks.filter((t) => t.participant.identity !== featuredTrack.participant.identity)
        : cameraTracks,
  ...
  return (
    <div className="flex flex-col h-full bg-[#07080f]">
      ...
      {activeReactions.map((r) => (
        <FloatingReaction key={r.id} emoji={r.emoji} id={r.id} senderName={r.senderName} />
      ))}
      ...
      <div className="flex items-center justify-between px-5 py-2.5 border-b border-indigo-500/10 bg-[#07080f]/90 backdrop-blur-xl shrink-0 z-10">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            {isConnected && (
              <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-emerald-500 rounded-full border border-[#07080f] animate-pulse" />
            )}
          </div>
          <div>
            ...
            <p className="text-[9px] text-indigo-400/60 font-bold uppercase tracking-wider">
              {userName}
            </p>
          </div>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 rounded-full transition-colors group"
          >
            <Share2
              size={10}
              className="text-indigo-400 group-hover:scale-110 transition-transform"
            />
            <span className="text-[9px] text-indigo-300 font-black uppercase tracking-widest">
              Invitar
            </span>
          </button>
          <span className="text-[9px] text-indigo-300/40 font-mono uppercase tracking-widest px-3 py-1 bg-indigo-500/6 border border-indigo-500/12 rounded-full">
            ROOM · {roomId.split('-')[0].toUpperCase()}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/3 border border-white/7">
            ...
            <span className="text-[9px] text-white/40 font-bold">{participants.length + 1}</span>
          </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex-1 min-w-0 relative overflow-hidden">
          ...
          <div className="relative z-10 h-full">
            {viewMode === 'grid' ? (
              <GridLayout
                tracks={[...cameraTracks, ...screenTracks]}
                pinnedId={pinnedId}
                raisedHands={finalRaisedHands}
                onPin={setPinnedId}
              />
            ) : (
              <SpotlightLayout
