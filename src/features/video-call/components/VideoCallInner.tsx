...
import {
    useParticipants, useConnectionState, useTracks, useLocalParticipant, useRoomContext, RoomAudioRenderer
...
export function VideoCallInner({ roomId, userName, isHost, onLeave }: { roomId: string; userName: string; isHost: boolean; onLeave: () => void }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[#08090f]">
            ...
            <div className="flex items-center justify-center flex-wrap gap-2 px-6 py-3 bg-[#04050a] border-t border-violet-500/10 shrink-0 relative z-[999]">
                ...
                {/* Leave */}
                <button onClick={() => {
                    if (isHost) setLeaveModalOpen(true);
                    else onLeave();
                }}
            ...
            {leaveModalOpen && isHost && (
                <div className="fixed inset-0 bg-black/80 z-[999999] flex items-center justify-center p-4">
                    <div className="bg-[#0e0f1e] border border-violet-500/20 rounded-2xl p-6 max-w-sm w-full text-center">
                        <h3 className="text-white font-bold text-lg mb-2">Terminar videollamada</h3>
                        <p className="text-white/60 text-sm mb-6">Al terminar la llamada, se desconectará a todos los participantes de forma permanente.</p>
                        <div className="flex flex-col gap-3">
                            <button onClick={async () => {
                                try {
                                    await apiClient.post(`/video-call/rooms/by-room/${roomId}/end`);
                                    const str = JSON.stringify({ type: 'room_ended' });
                                    localParticipant.publishData(new TextEncoder().encode(str), { reliable: true });
                                } catch { }
                                onLeave();
                            }}
                                className="w-full py-3 bg-red-600 hover:bg-red-500 text-white font-black uppercase tracking-wider text-xs rounded-xl transition-colors">
                                Terminar para todos
                            </button>
                            <button onClick={onLeave}
                                className="w-full py-3 bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-wider text-xs rounded-xl transition-colors">
                                Solo salir de la sala
                            </button>
                            <button onClick={() => setLeaveModalOpen(false)}
                                className="w-full py-2 mt-2 text-white/40 hover:text-white text-xs font-bold uppercase tracking-wider uppercase transition-colors">
                                Cancelar
                            </button>
                        </div>
                    </div>
