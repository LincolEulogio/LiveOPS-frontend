...
export function ChatPanel({ onClose, messages, onSend }: { onClose: () => void; messages: any[]; onSend: (text: string) => void }) {
    ...
    return (
        <div className="flex flex-col w-[300px] shrink-0 bg-[#0a0b14] border-l border-indigo-500/10">
            <div className="flex items-center gap-3 px-4 py-3 border-b border-indigo-500/10 bg-[#0d0e1c] shrink-0">
                ...
                <span className="text-indigo-300/70 text-[10px] font-black uppercase tracking-[0.15em]">Chat</span>
                ...
                {messages.map((msg: any) => {
                    ...
                    return (
                        <div key={msg.id ?? msg.timestamp} className={`flex flex-col max-w-[85%] gap-0.5 ${isOwn ? 'self-end items-end' : 'self-start items-start'}`}>
                            {!isOwn && <span className="text-indigo-300/60 text-[9px] font-black uppercase tracking-widest px-1">{msg.from?.name || msg.from?.identity}</span>}
            ...
            <div className="px-3 py-3 border-t border-indigo-500/10 bg-[#0d0e1c] flex gap-2 items-end shrink-0">
                ...
                <button onClick={handleSend} disabled={!input.trim()}
                    className="w-10 h-10 min-w-[40px] rounded-full bg-gradient-to-br from-indigo-700 to-indigo-500 flex items-center justify-center shadow-lg shadow-indigo-500/25 disabled:opacity-40 transition-opacity">
