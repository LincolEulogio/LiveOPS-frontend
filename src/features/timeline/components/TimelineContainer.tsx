...
export const TimelineContainer = ({ productionId }: Props) => {
  ...
  return (
    <div className="bg-card-bg/60 backdrop-blur-2xl border border-card-border rounded-[2.5rem] overflow-hidden  flex flex-col group/timeline h-full shadow-lg">
      ...
      <div className="p-6 sm:p-10 border-b border-card-border/50 bg-white/5 dark:bg-black/20 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-5">
          <div className="w-14 h-14 bg-indigo-600/10 rounded-2xl flex items-center justify-center border border-indigo-500/20 shadow-inner">
            <Layout className="text-indigo-600 dark:text-indigo-400" size={28} />
          </div>
          <div>
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 text-[9px] font-black text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 transition-colors uppercase tracking-widest',
                  isAiLoading && 'animate-pulse'
                )}
              >
                <Sparkles size={12} className={cn(isAiLoading && 'animate-spin')} />
                AI Insights
              </button>
            </div>
          </div>
        ...
        <div className="relative z-10 flex items-center gap-3 w-full lg:w-auto">
          {canControl && (
            <div className="flex items-center gap-2 flex-1 lg:flex-none">
              <motion.button
                ...
                disabled={!hasNext}
                className={cn(
                  'flex-1 lg:flex-none flex items-center justify-center gap-3 px-6 py-4 rounded-xl transition-all text-[11px] font-black uppercase tracking-widest shadow-lg ',
                  hasNext
              ...
              >
                <ArrowRight size={18} className={hasNext ? 'animate-pulse' : ''} />
                {hasActive ? 'Deploy Next' : 'Engage Systems'}
              </motion.button>

              <button
                onClick={handleResetAll}
                className="p-4 bg-card-bg/80 hover:bg-orange-500/10 text-muted hover:text-orange-500 rounded-xl border border-card-border transition-all active:scale-95"
          ...
          {canEdit && (
            <button
              ...
              className="flex-1 lg:flex-none flex items-center justify-center gap-3 bg-foreground text-background dark:bg-white dark:text-black hover:opacity-90 px-6 py-4 rounded-xl transition-all text-[11px] font-black uppercase tracking-widest shadow-xl active:scale-95"
            >
              <Plus size={18} />
              Add Segment
            </button>
