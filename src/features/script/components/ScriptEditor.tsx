...
export const ScriptEditor = ({ productionId }: Props) => {
  ...
  const handleSendAi = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api/v1'}/ai/chat-stream`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            history: [...aiMessages, { role: 'user', content: userMsg }],
      ...
      setAiMessages((prev) => [
        ...prev,
        { role: 'assistant', content: 'ALERT: Failed to reach creative intelligence nodes.' },
      ]);
    } finally {
  ...
  const canEdit = usePermissions(['script:edit']);
  ...
  const editor = useEditor(
    {
      ...
      extensions: [
        ...
        Placeholder.configure({
          placeholder: canEdit
            ? 'Escribe el guion de la producción aquí... Usa [ESCENA:Nombre] u [BLOCK:Nombre] para crear disparadores.'
            : 'El guion está en modo lectura...',
        }),
        CharacterCount,
        SceneTag,
        TimelineTag,
      ...
      editorProps: {
        attributes: {
          class:
            'prose dark:prose-invert prose-sm sm:prose-base focus:outline-none max-w-none px-6 sm:px-12 py-10 sm:py-16 prose-p:my-1 prose-headings:my-2 prose-h1:text-4xl prose-h1:font-black prose-h2:text-3xl prose-h2:font-black cursor-text min-h-full font-serif',
        },
      },
      onSelectionUpdate: () => {
        setTick((t) => t + 1);
      ...
      <div className="flex-1 flex items-center justify-center bg-card-bg/50 rounded-3xl backdrop-blur-xl">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] font-bold text-muted uppercase  animate-pulse">
              Syncing with Cloud Doc...
            </p>
          </div>
        </div>
      </div>
