    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="h-20 flex items-center justify-between px-8 border-b border-card-border/50 bg-white/2 shrink-0">
        <NextLink href="/productions" className="flex items-center gap-3 group/logo">
          ...
          <div>
            <h1 className="text-xl font-black text-foreground er uppercase italic leading-none">
              LiveOPS
            </h1>
            <p className="text-[8px] font-black text-indigo-400 uppercase  mt-1 opacity-60">
              Control Surface
            </p>
          </div>
        </NextLink>
      ...
      <nav className="flex-1 overflow-y-auto py-8 px-5 space-y-2 custom-scrollbar min-h-0">
        <p className="px-4 text-[9px] font-black text-muted uppercase  mb-4 opacity-40">
          Core Systems
        </p>
        ...
        <Guard requiredPermissions={['user:manage', 'role:manage']}>
          <div className="h-8" />
          <p className="px-4 text-[9px] font-black text-indigo-400 uppercase  mb-4 opacity-60">
            Administration
          </p>
      ...
      <div className="p-6 bg-white/3 border-t border-card-border/50 shrink-0">
        <div className="p-4 bg-background/60 backdrop-blur-md rounded-2xl border border-card-border/60 flex items-center gap-4 mb-4 relative overflow-hidden group/user">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-black text-foreground truncate er uppercase leading-none mb-1">
              {user?.name || 'Operator'}
            </p>
            <div className="flex items-center gap-1.5 opacity-60">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <p className="text-[9px] font-bold text-muted truncate uppercase ">
                {user?.globalRole?.name || 'Authorized'}
              </p>
            </div>
          </div>
        </div>
...
const SidebarLink = ({ href, icon: Icon, label, active }: any) => (
  <NextLink
    ...
    className={cn(
      ...
      active ? 'bg-indigo-600 text-white' : 'text-muted hover:text-foreground hover:bg-white/5'
    )}
  >
    ...
    {active && <div className="w-1.5 h-1.5 rounded-full bg-white relative z-10" />}
    {active && <motion.div layoutId="nav-glow" className="absolute inset-0 bg-white/10" />}
  </NextLink>
);
