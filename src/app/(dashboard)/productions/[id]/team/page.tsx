    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center p-12 space-y-4">
                ...
                <p className="text-stone-400 animate-pulse">Loading team data...</p>
            </div>
        );
    }
    ...
    if (fetchError || !production) {
        return (
            <div className="bg-red-500/10 text-red-400 p-6 rounded-xl border border-red-500/20 max-w-2xl mx-auto mt-8 flex flex-col items-center text-center">
                ...
                <Link href={`/productions/${id}`} className="px-4 py-2 bg-stone-900 text-white rounded hover:bg-stone-800 transition-colors">
                    Back to Production
                </Link>
            </div>
        );
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 pb-12">
            ...
            <div>
                ...
                <p className="text-stone-400 mt-2">Manage access and roles for this production environment</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Assignment Form */}
                <div className="lg:col-span-1">
                    <div className="bg-stone-900 border border-stone-800 rounded-2xl p-6 sticky top-6 shadow-xl">
                    ...
                    <div className="bg-stone-900 border border-stone-800 rounded-2xl overflow-hidden shadow-xl">
                        <div className="p-6 border-b border-stone-800">
                            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
                                ...
                                Active Members
                                <span className="ml-2 px-2 py-0.5 bg-stone-800 rounded text-xs text-stone-400 font-normal">
                        ...
                        <ul className="divide-y divide-stone-800/50">
                            {((production as any).users || []).map((prodUser: any) => (
                                <li key={prodUser.userId} className="p-4 hover:bg-stone-800/30 transition-colors flex items-center justify-between group">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-stone-950 border border-stone-800 flex items-center justify-center text-stone-400">
                                            <User size={20} />
                                        </div>
                                        <div>
                                            <h4 className="text-white font-medium text-sm">{prodUser.user.name || 'Unnamed User'}</h4>
                                            <p className="text-xs text-stone-500">{prodUser.user.email}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <span className="px-2.5 py-1 bg-stone-950 border border-stone-800 rounded-md text-[10px] font-bold uppercase tracking-widest text-indigo-400">
                                            {prodUser.role.name}
                                        </span>
                                        ...
                                        <button
                                            ...
                                            className="p-2 text-stone-600 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 disabled:opacity-0"
                                            title="Remove member"
                                        >
                                            <UserMinus size={18} />
                                        </button>
                            ...
                            {(!(production as any).users || (production as any).users.length === 0) && (
                                <li className="p-12 text-center text-stone-500">
