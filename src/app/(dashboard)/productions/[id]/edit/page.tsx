    ...
    try {
      ...
      if (formData.vmixConfig?.host && formData.vmixConfig.host.includes('://')) {
        try {
          ...
          formData.vmixConfig.host = formData.vmixConfig.host
            .replace(/^http(s)?:\/\//, '')
            .split('/')[0]
            .split(':')[0];
        }
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-[10px] font-mono text-indigo-400">
                        {watch('vmixConfig.pollingInterval')}ms
                      </span>
                    </div>
                    <input
                    ...
                    <p className="text-[10px] text-muted">
                      Frequency of API requests to vMix. Lower values provide faster feedback but
                      increase CPU/Network load.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="mt-10 pt-10 border-t border-card-border">
            ...
            <div className="flex gap-2 mb-6">
              <div className="flex-1">
                ...
                >
                  ...
                  {globalUsers
                    ?.filter((u) => !production?.users?.some((pu: any) => pu.userId === u.id))
                    .map((u) => (
            ...
            <div className="space-y-2 bg-background border border-card-border rounded-2xl p-4">
              {production?.users?.map((pu: any) => (
                <div
                  key={pu.userId}
                  className="flex items-center justify-between py-2 px-3 hover:bg-card-bg/50 rounded-xl transition-colors group"
                >
