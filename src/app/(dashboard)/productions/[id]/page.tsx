...
export default function ProductionDetailPage() {
  ...
  if (error || !production) {
    return (
      <div className="bg-red-500/10 text-red-400 p-6 rounded-xl border border-red-500/20 max-w-2xl mx-auto mt-8 flex flex-col items-center text-center">
        ...
        <Link
          href="/productions"
          className="px-4 py-2 bg-background border border-card-border text-foreground rounded hover:bg-card-bg transition-colors"
        >
          Return to List
        </Link>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-card-bg border border-card-border p-6 rounded-2xl shadow-xl gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-background rounded border border-card-border">
            ...
            <h1 className="text-3xl font-bold text-foreground tracking-tight">{production.name}</h1>
          </div>
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="p-2.5 bg-background border border-card-border rounded-xl hover:bg-card-border transition-all text-muted hover:text-foreground"
            title="Edit Production"
          >
            <Settings size={20} />
          </Link>
          <span className="px-3 py-1 bg-background border border-card-border rounded-full text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                <Link
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-card-bg border border-card-border rounded-xl text-[10px] font-bold text-foreground uppercase tracking-widest hover:bg-card-border hover:border-indigo-500/30 transition-all shadow-lg"
                >
                ...
                <Link
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-background border border-card-border rounded-xl text-[10px] font-bold text-foreground uppercase tracking-widest hover:bg-card-border hover:border-emerald-500/30 transition-all shadow-lg"
                >
        ...
        <div className="space-y-8 h-full">
          ...
          <div className="bg-card-bg border border-card-border rounded-2xl p-6 shadow-lg">
            ...
            <div className="space-y-4 mb-8">
              {production.users?.slice(0, 3).map((u: any) => (
                <div key={u.userId} className="flex items-center gap-3 group">
                  <div className="w-8 h-8 rounded-full bg-background border border-card-border flex items-center justify-center text-[10px] font-bold text-muted group-hover:border-indigo-500/50 transition-colors">
                    {u.user.name?.[0]?.toUpperCase() || 'U'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground truncate">{u.user.name || u.user.email}</p>
                    <p className="text-[10px] text-muted uppercase tracking-widest font-bold">{u.role.name}</p>
                  </div>
                </div>
              ))}
              ...
              {production.users && production.users.length > 3 && (
                <p className="text-[10px] text-muted font-medium pl-11">
            ...
            <Guard requiredPermissions={['production:manage']}>
              <Link
                ...
                className="block w-full text-center px-4 py-2.5 bg-background hover:bg-card-border border border-card-border text-foreground text-xs font-bold uppercase tracking-widest rounded-xl transition-all hover:border-indigo-500/30"
              >
                Manage Team
              </Link>
            </Guard>
          ...
          <div className="bg-card-bg border border-card-border rounded-2xl p-6 shadow-lg">
            ...
            <Link
              ...
              className="block w-full text-center px-4 py-2 bg-background hover:bg-card-border border border-card-border text-foreground text-sm font-semibold rounded-lg transition-all"
            >
              Open Engine
            </Link>
          </div>
