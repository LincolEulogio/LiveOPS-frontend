    ...
    return (
      <div className="bg-red-500/10 text-red-400 p-12 rounded-[2.5rem] border border-red-500/20 max-w-2xl mx-auto mt-12 flex flex-col items-center text-center backdrop-blur-xl">
        ...
        <p className="mb-8 font-medium opacity-60">
          The production you are looking for does not exist or you lack sufficient access
          permissions.
        </p>
        <Link
          href="/productions"
  ...
  return (
    <div className="mx-auto max-w-[1800px] space-y-6 sm:space-y-8 pb-6 mt-2 sm:mt-6 px-4 sm:px-0">
      ...
      <div className="grid grid-cols-1 min-[1280px]:grid-cols-12 gap-8">
        ...
        <div
          className={cn(
            'min-[1280px]:col-span-8 min-[1440px]:col-span-9 space-y-8',
            activeTab !== 'overview' && 'block min-[1280px]:block'
          )}
        >
          {/* Section: Streaming Dashboard (Primary) */}
          <section
            className={cn(
              activeTab !== 'streaming' && activeTab !== 'overview' && 'hidden lg:block'
            )}
          >
          ...
          {/* Section: Guion Vivo */}
          <section
            className={cn(activeTab !== 'script' && activeTab !== 'overview' && 'hidden lg:block')}
          >
          ...
          {/* Section: Social Feed */}
          <section
            className={cn(activeTab !== 'social' && activeTab !== 'overview' && 'hidden lg:block')}
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-black text-foreground uppercase  flex items-center gap-3">
            ...
            <Guard
              requiredPermissions={['social:manage']}
              fallback={
                <div className="h-[450px] bg-card-bg/30 rounded-2xl overflow-hidden grayscale pointer-events-none opacity-50 relative group">
                  <div className="absolute inset-0 z-10 flex items-center justify-center">
                    <p className="text-[10px] font-black uppercase text-white bg-stone-900 border border-stone-800 px-4 py-2 rounded-xl">
          ...
          <div
            className={cn(
              'grid grid-cols-1 min-[1500px]:grid-cols-2 gap-8',
              activeTab !== 'overview' && 'hidden min-[1280px]:grid'
            )}
          >
            ...
            <div className="bg-card-bg/80 backdrop-blur-xl border border-card-border rounded-2xl p-6  relative overflow-hidden flex flex-col h-full">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="space-y-1">
                  <h2 className="text-xs font-black text-muted uppercase tracking-tighter">
                    Live Team
                  </h2>
              ...
              <div className="space-y-4 mb-8 flex-1">
                {production.users?.slice(0, 4).map((u: any) => (
                  <div
                    key={u.userId}
                    className="flex items-center gap-3 p-2 hover:bg-white/5 rounded-2xl transition-colors cursor-default border border-transparent hover:border-white/5"
                  >
              ...
              <Link href={`/productions/${id}/analytics`} className="group relative block h-full">
                <div className="bg-card-bg/80 backdrop-blur-xl border border-card-border rounded-2xl p-8 h-full transition-all hover:bg-background/80 hover:border-pink-500/40 relative overflow-hidden active:scale-[0.98] flex flex-col justify-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted font-bold leading-relaxed uppercase group-hover:text-muted-foreground transition-colors">
                      Insights, Viewership & Performance Metrics
                    </p>
                  </div>
              ...
              <Link href={`/productions/${id}/automation`} className="group relative block h-full">
                <div className="bg-card-bg/80 backdrop-blur-xl border border-card-border rounded-2xl p-8 h-full transition-all hover:bg-background/80 hover:border-indigo-500/40 relative overflow-hidden active:scale-[0.98] flex flex-col justify-center">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted font-bold leading-relaxed uppercase group-hover:text-muted-foreground transition-colors">
                      Logic Rules, Cloud Hooks & Macro Triggers
                    </p>
                  </div>
        ...
        <div
          className={cn(
            'min-[1280px]:col-span-4 min-[1440px]:col-span-3 space-y-8',
            activeTab !== 'overview' && 'hidden min-[1280px]:block'
          )}
        >
          <div className="lg:sticky lg:top-8 space-y-8 pb-20">
            ...
            {/* Sidebar Media */}
            <Guard requiredPermissions={['media:view']}>
