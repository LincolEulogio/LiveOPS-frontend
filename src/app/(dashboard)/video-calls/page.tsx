...
function StatusBadge({ status }: { status: string }) {
    ...
    const dots = { scheduled: 'bg-amber-500', active: 'bg-emerald-500 animate-pulse', ended: 'bg-white/20' }[status] || 'bg-white/20';

    return (
        <span className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[9px] font-black uppercase tracking-wider ${styles}`}>
...
function CopyLink({ roomId }: { roomId: string }) {
    ...
    return (
        <button onClick={copy} title="Copiar link de invitación"
...
/* ─── Modal: Create / Edit ──────────────────────────────────────────────── */
function CallModal({ call, onClose, onSave }: {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const body = { title: title.trim(), description: description.trim() || undefined, scheduledAt: scheduledAt || undefined };
            if (call) {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="w-full max-w-md bg-[#0d0e1c] border border-violet-500/15 rounded-3xl p-6 shadow-2xl">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={onClose} className="p-1.5 rounded-lg bg-white/5 border border-white/8 text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors">
                        <X size={14} />
                    </button>
                </div>
                ...
                <div className="space-y-4">
                    <div>
                        <label className="block text-[9px] font-black text-white/40 uppercase tracking-wider mb-1.5">Título *</label>
                        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Reunión semanal de equipo"
                    ...
                    <div>
                        <label className="block text-[9px] font-black text-white/40 uppercase tracking-wider mb-1.5">Descripción</label>
                        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Agenda, temas a tratar..." rows={3}
                ...
                <div className="flex gap-3 mt-6">
                    <button onClick={onClose} className="flex-1 py-2.5 bg-white/5 hover:bg-white/8 border border-white/10 rounded-xl text-white/60 text-[10px] font-black uppercase tracking-widest transition-colors">
                        Cancelar
                    </button>
...
/* ─── CallCard ──────────────────────────────────────────────────────────── */
function CallCard({ call, onEdit, onDelete, onJoin, isOwn }: {
    call: VideoCallItem;
    onEdit: () => void;
    onDelete: () => void;
    onJoin: () => void;
    ...
    return (
        <div className={`relative bg-[#0d0e1c] border rounded-2xl p-5 flex flex-col gap-4 transition-all hover:border-violet-500/25 hover:shadow-lg hover:shadow-violet-500/5 ${call.status === 'active' ? 'border-emerald-500/25 shadow-emerald-500/5 shadow-lg' : 'border-violet-500/10'
            }`}>
            {/* Active indicator */}
            {call.status === 'active' && (
            ...
            <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${call.status === 'active' ? 'bg-emerald-500/15 border border-emerald-500/25' : 'bg-violet-500/10 border border-violet-500/15'
                    }`}>
                    <Video size={16} className={call.status === 'active' ? 'text-emerald-400' : 'text-violet-400'} />
                </div>
            ...
            {call.description && (
                <p className="text-[12px] text-white/50 leading-relaxed border-t border-white/5 pt-3">{call.description}</p>
            )}

            {scheduled && (
                <div className={`flex items-center gap-2 text-[11px] font-bold ${isPast ? 'text-red-400/70' : 'text-amber-300/70'}`}>
                    <Calendar size={11} />{scheduled.toLocaleDateString()} · <Clock size={11} />{scheduled.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            ...
            <div className="flex items-center gap-2 pt-1 border-t border-white/5">
                ...
                <div className="ml-auto flex items-center gap-1.5">
                    ...
                    <button onClick={onJoin}
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${call.status === 'active'
                        ...
                        <Play size={11} />{call.status === 'active' ? 'Unirse' : 'Iniciar'}
                    </button>
                </div>
            </div>
        </div>
...
export default function VideoCallsPage() {
    ...
    const [modal, setModal] = useState<{ open: boolean; call?: VideoCallItem | null }>({ open: false });
    ...
    return (
        <div className="min-h-screen bg-[#07080f] text-white">
            {modal.open && (
                <CallModal call={modal.call} onClose={() => setModal({ open: false })} onSave={loadCalls} />
            )}
            ...
            <div className="border-b border-violet-500/10 bg-[#07080f]/80 backdrop-blur-xl sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-[11px] font-black text-white uppercase tracking-widest">Video Calls</h1>
                            <p className="text-[9px] text-violet-400/50 uppercase tracking-wider">
                                {active.length} activa{active.length !== 1 ? 's' : ''} · {scheduled.length} programada{scheduled.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                    </div>
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                    {[
                        { label: 'En Vivo', value: active.length, color: 'emerald', icon: '🟢', filter: 'active' },
                        { label: 'Programadas', value: scheduled.length, color: 'amber', icon: '📅', filter: 'scheduled' },
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-[9px] text-white/40 font-black uppercase tracking-wider">{stat.label}</p>
                            </div>
                        </button>
                    ))}
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-6">
                    <div className="relative flex-1 max-w-xs">
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar videollamadas..."
                    ...
                    <div className="flex gap-2">
                        {(['all', 'active', 'scheduled'] as const).map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${filter === f ? 'bg-violet-600/90 border border-violet-500/50 text-white' : 'bg-white/5 border border-white/8 text-white/50 hover:text-white/70'
                                    }`}>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <p className="text-white/30 text-sm text-center">
                            {search ? 'No se encontraron resultados' : 'No hay videollamadas aún'}<br />
                        ...
                        <button onClick={() => setModal({ open: true })}
                            ...
                            <Plus size={12} />Nueva Videollamada
                        </button>
                    </div>
                ) : (
