                                    ...
                                    <td className="py-6 px-8">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted-foreground dark:text-muted font-mono opacity-50">{user.email}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-6 px-8">
                                        {user.globalRole ? (
                                            ...
                                            <span className="text-[10px] text-muted-foreground dark:text-muted italic opacity-40 uppercase tracking-widest">Restricted Access</span>
                                        )}
                                    </td>
                                    <td className="py-6 px-8">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-foreground dark:text-white/70 uppercase">{new Date(user.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                            <span className="text-[8px] text-muted-foreground dark:text-muted uppercase tracking-tighter">System Initialized</span>
                                        </div>
                                    </td>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-5 p-5 bg-gray-50 dark:bg-white/2 border border-black/5 dark:border-white/5 rounded-2xl group transition-all">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[9px] font-black text-muted uppercase tracking-widest leading-none mb-1">Authorization Email</p>
                                            <p className="text-sm font-bold text-foreground dark:text-white">{selectedUser.email}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-5 p-5 bg-gray-50 dark:bg-white/2 border border-black/5 dark:border-white/5 rounded-2xl group transition-all">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[9px] font-black text-muted uppercase tracking-widest leading-none mb-1">Service Initiation</p>
                                            <p className="text-sm font-bold text-foreground dark:text-white">{new Date(selectedUser.createdAt).toLocaleDateString(undefined, { dateStyle: 'long' })}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-5 p-5 bg-gray-50 dark:bg-white/2 border border-black/5 dark:border-white/5 rounded-2xl group transition-all">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[9px] font-black text-muted uppercase tracking-widest leading-none mb-1">Matrix Identifier</p>
                                            <p className="text-[10px] font-mono text-muted group-hover:text-indigo-400/60 transition-colors uppercase">{selectedUser.id}</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            >
                                ...
                                <form onSubmit={handleSave} className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-indigo-600/60 dark:text-indigo-400/60 uppercase tracking-widest pl-1 flex items-center gap-1.5"><AtSign size={10} /> Designation (Full Name)</label>
                                        <input
                                            type="text"
                                            required
                                            value={form.name}
                                    ...
                                    <div className="grid grid-cols-1 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-indigo-600/60 dark:text-indigo-400/60 uppercase tracking-widest pl-1 flex items-center gap-1.5"><Mail size={10} /> Auth Email</label>
                                            <input
                                                type="email"
                                                required
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-indigo-600/60 dark:text-indigo-400/60 uppercase tracking-widest pl-1 flex items-center gap-1.5"><KeyRound size={10} /> {modalMode === 'create' ? 'Access Key (Password)' : 'Override Key (Leave blank to keep current)'}</label>
                                        <input
                                            type="password"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-indigo-600/60 dark:text-indigo-400/60 uppercase tracking-widest pl-1 flex items-center gap-1.5"><Shield size={10} /> Clearance Level (Global Role)</label>
                                        <select
                                        ...
                                        >
                                            <option value="" className="bg-white dark:bg-[#0a0a0f] text-foreground dark:text-white">NO CLEARANCE</option>
                                            {roles?.map(role => (
                                                <option key={role.id} value={role.id} className="bg-white dark:bg-[#0a0a0f] text-foreground dark:text-white lowercase first-letter:uppercase">{role.name}</option>
                                            ))}
                                        </select>
                                    </div>
